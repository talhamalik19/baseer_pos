...
export async function magentoGraphQLFetch({
  ...
  try {
    ...
    // 🔥 Inject `next` object for Next.js fetch options
    ...
    const result = await fetch(`${endpoint}/graphql`, fetchConfig);
...
export async function getProducts({ id, sort = ``, currency }) {
...
export async function recentOrders() {
  const cookieStore = await cookies();
  const jwt = cookieStore.get("jwt")?.value;
  const query = getRecentOrderQuery;
  ...
  try {
    const response = await magentoGraphQLFetch({ query, variables, headers });
    if (response?.body?.data?.getRecentOrderItems) {
      return {
        status: response.status,
        data: response?.body?.data?.getRecentOrderItems?.data,
      };
    } else {
      return {
        errors: response?.errors,
