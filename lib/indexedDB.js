...
const openDB = () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(REFUNDS_STORE)) {
        db.createObjectStore(REFUNDS_STORE, { keyPath: "id", autoIncrement: true });
      }
...
export const updateCartItemQuantity = async (cartItemUid, newQuantity) => {
  ...
  try {
    ...
    // First, try to get the item by its uid (which is the keyPath)
    ...
    if (item) {
      ...
      // If not found by uid, search through all items (fallback)
      ...
      // Try to find item by product uid or other identifier
      const itemToUpdate = allItems.find(item =>
        item.uid === cartItemUid || // Check the cart item uid
        item.product?.uid === cartItemUid // Check the product uid
      );
  ...
  try {
    ...
    for (const customer of customers) {
      if (customer.id || customer.customer_id) {
        const item = { ...customer, id: customer.id || customer.customer_id };
        store.put(item);
      }
    }
    await tx.done;
