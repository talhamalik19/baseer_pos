  ...
  try {
    ...
    if (existingItem) {
      ...
      const newItem = {
        ...
        addedAt: Date.now(),
        taxAmount: taxAmount,
        uid: Date.now() // Add a unique identifier
      };
      await new Promise((resolve, reject) => {
        const putRequest = store.put(newItem);
        putRequest.onsuccess = () => resolve();
...
export const removeFromCart = async (productUid) => {
  if (typeof window === "undefined") return false;
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      getAllRequest.onsuccess = () => {
        const itemToRemove = getAllRequest.result.find(
        ...
        if (itemToRemove) {
          if (itemToRemove.quantity > 1) {
            ...
            const deleteRequest = store.delete(itemToRemove.uid);
      ...
      getAllRequest.onerror = () => reject(false);
    });
...
export const deleteFromCart = async (productUid) => {
  if (typeof window === "undefined") return false;
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      getAllRequest.onsuccess = () => {
        const itemToDelete = getAllRequest.result.find(
          item => item.product.uid === productUid
        );

        if (itemToDelete) {
          const deleteRequest = store.delete(itemToDelete.uid);
          deleteRequest.onsuccess = () => resolve(true);
          deleteRequest.onerror = () => resolve(false);
        } else {
...
export const updateCartItemQuantity = async (productUid, addedAt, newQuantity) => {
  if (typeof window === "undefined") return false;
  ...
  try {
    ...
    const updatedItem = await new Promise((resolve, reject) => {
      getAllRequest.onsuccess = async () => {
        const cartItems = getAllRequest.result; // Wait until request is done
        // Find the exact match using UID + Options
...
export const getViewMode = async () => {
  if (typeof window === "undefined") return "cards"; // Default mode
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => resolve(request.result?.mode || "cards");
      request.onerror = (event) => reject(event.target.error);
    });
  } catch (error) {
...
export const updateWholeProduct = async (productUid, addedAt, newItem) => {
  if (typeof window === "undefined") return false;
  ...
  try {
    ...
    const success = await new Promise((resolve, reject) => {
      getAllRequest.onsuccess = () => {
        ...
        const existingItem = cartItems.find(
          item => item.product.uid === productUid && item?.addedAt === addedAt
        );

        if (existingItem) {
          // Replace the item completely with newItem
          const putRequest = store.put(newItem);
          putRequest.onsuccess = () => resolve(true);
          putRequest.onerror = (event) => reject(event.target.error);
        } else {
