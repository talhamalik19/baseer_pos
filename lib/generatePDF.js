...
const generateReceiptPDF = async (
    ...
    try {
      const logoDataUrl = await new Promise((resolve, reject) => {
        ...
        img.onload = function () {
          ...
          const maxWidth = 60; // Changed from 150 to 60
          const maxHeight = 32; // Changed from 80 to 32
  ...
  const itemsTable = cart.map((item) => {
    ...
    let priceDisplay = `$${actualPrice.toFixed(2)}`;
  ...
  if (totalProductDiscount > 0 && !hasCartDiscount) {
    ...
    doc.text(`-$${totalProductDiscount.toFixed(2)}`, 190, y, { align: "right" });
    y += 5;
  }
  ...
  doc.text(`$${subtotalWithTax.toFixed(2)}`, 190, y, { align: "right" });
  y += 5;
  ...
  if (hasCartDiscount && cartDiscountAmount > 0) {
    ...
    doc.text(`-$${cartDiscountAmount.toFixed(2)}`, 190, y, { align: "right" });
    y += 5;
  }
