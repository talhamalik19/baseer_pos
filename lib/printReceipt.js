...
export const printReceipt = async (
      ...
      if (hasDiscount) {
        receiptContent += `
          <span style="width: 100px; text-align: right;">
            <span class="strikethrough">${currencySymbol}${regularPrice.toFixed(2)}</span><br/>
            ${currencySymbol}${actualPrice.toFixed(2)}
          </span>
        `;
      } else {
        receiptContent += `
          <span style="width: 100px; text-align: right;">${currencySymbol}${actualPrice.toFixed(2)}</span>
        `;
      }
    } else {
      receiptContent += `
        <span style="width: 60px; text-align: right;">${currencySymbol}${actualPrice.toFixed(2)}</span>
      `;
    }

    receiptContent += `
        <span style="width: 60px; text-align: right;">${currencySymbol}${taxAmount.toFixed(2)}</span>
        <span style="width: 70px; text-align: right;">${currencySymbol}${total.toFixed(2)}</span>
      </div>
    `;
  });
  ...
  receiptContent += `
      ...
      <div class="totals">
        <div class="totals-line"><span>Subtotal (Excl. Tax):</span><span>${currencySymbol}${subtotalWithoutTax.toFixed(2)}</span></div>
  `;

  // Show product-level discount if applicable
  if (totalProductDiscount > 0 && !hasCartDiscount) {
    receiptContent += `
        <div class="totals-line"><span>Product Discount:</span><span>-${currencySymbol}${totalProductDiscount.toFixed(2)}</span></div>
    `;
  }

  receiptContent += `
        <div class="totals-line"><span>Tax:</span><span>${currencySymbol}${totalTax.toFixed(2)}</span></div>
        <div class="totals-line"><span>Subtotal (Incl. Tax):</span><span>${currencySymbol}${subtotalWithTax.toFixed(2)}</span></div>
  `;

  // Show cart-level discount if applicable
  if (hasCartDiscount && cartDiscountAmount > 0) {
    receiptContent += `
        <div class="totals-line"><span>Cart Discount (${cartDiscountPercent.toFixed(1)}%):</span><span>-${currencySymbol}${cartDiscountAmount.toFixed(2)}</span></div>
    `;
  }

  receiptContent += `
        <div class="totals-line"><strong>Total:</strong><strong>${currencySymbol}${grandTotal.toFixed(2)}</strong></div>
        <div class="totals-line"><span>Amount Paid:</span><span>${currencySymbol}${parseFloat(amountPaid || 0).toFixed(2)}</span></div>
        <div class="totals-line"><span>Change:</span><span>${currencySymbol}${parseFloat(balance || 0).toFixed(2)}</span></div>
      </div>
  `;
