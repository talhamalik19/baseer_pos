...
export const printReceipt = async (
  ...
  let receiptContent = `
    ...
    <head>
      ...
      <style>
        ...
        .item-header, .item { display: flex; justify-content: space-between; font-size: 12px; }
        .item-header { font-weight: bold; border-bottom: 1px solid #000; padding-bottom: 5px; margin-bottom: 5px; }
  ...
  cart.forEach((item) => {
    ...
    receiptContent += `
      <div class="item">
        <span style="flex: 1;">${name}</span>
        <span style="width: 40px; text-align: right;">${qty}</span>
        <span style="width: 60px; text-align: right;">$${basePrice.toFixed(2)}</span>
  ...
  receiptContent += `
      ...
      <div class="totals">
        ...
        ${
          cartDiscount > 0
            ? `<div class="totals-line"><span>Cart Discount (${discountPercent.toFixed(1)}%):</span><span>-$${cartDiscount.toFixed(2)}</span></div>`
            : ""
        }
        <div class="totals-line"><span>Tax:</span><span>$${totalTax.toFixed(2)}</span></div>
        <div class="totals-line"><strong>Total (Incl. Tax):</strong><strong>$${grandTotal.toFixed(2)}</strong></div>
