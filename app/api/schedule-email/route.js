...
if (phone && smsJob) {
  ...
  try {
    ...
    if (!res.ok) {
      writeToLogFile(`SMS API returned status: ${res.status}`);
    }
    
    const responseText = await res.text();
    writeToLogFile(`SMS API raw response: ${responseText}`);
    
    if (responseText) {
      try {
        const result = JSON.parse(responseText);
        writeToLogFile(`SMS success: ${result?.success}`);
        writeToLogFile(`SMS error: ${result?.error}`);
      } catch (parseError) {
        writeToLogFile(`Failed to parse SMS response as JSON: ${parseError.message}`);
      }
    } else {
      writeToLogFile(`SMS API returned empty response`);
    }
    
    writeToLogFile(`SMS sent successfully for order: ${orderId}`);
  } catch (smsError) {
    writeToLogFile(`SMS sending failed: ${smsError.message}`);
  }
}
    } else {
