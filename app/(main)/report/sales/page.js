...
export default async function SalesReports() {
  ...
  const submitSalesReport = async (apiUrl) => {
    "use server";
  ...
  return (
    <>
      <PageHead
        pageName={
          serverLanguage?.csvTranslations?.sales_report ?? "Sales Report"
        }
        firstName={firstname}
        lastName={lastname}
        initials={initials}
      ...
      />
      <SalesReport submitSalesReport={submitSalesReport} storeCode={storeCode} storeIds={storeIds}/>
    </>
  );
}
