...
export default async function SalesReports() {
  ...
  const submitInvoiceReport = async (apiUrl) => {
    "use server";
  ...
  return (
    <>
      <PageHead
        pageName={
          serverLanguage?.csvTranslations?.invoice_report ?? "Invoice Report"
        }
        firstName={firstname}
        lastName={lastname}
        initials={initials}
      ...
      />
      <InvoiceReport submitInvoiceReport={submitInvoiceReport} storeCode={storeCode} storeIds={storeIds}/>
    </>
  );
}
