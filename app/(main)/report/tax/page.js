...
import TaxReport from "@/components/blocks/Reports/TaxReport";
...
export default async function SalesReports() {
  ...
  const submitTaxReport = async (apiUrl) => {
    "use server";
  ...
  return (
    <>
      <PageHead
        pageName={
          serverLanguage?.csvTranslations?.tax_report ?? "Tax Report"
        }
        firstName={firstname}
        lastName={lastname}
        initials={initials}
      ...
      />
      <TaxReport submitTaxReport={submitTaxReport} storeCode={storeCode} storeIds={storeIds}/>
    </>
  );
}
