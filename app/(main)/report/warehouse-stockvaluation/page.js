...
import WarehouseValuationReport from "@/components/blocks/Reports/WarehouseValuationReport";

export default async function WarehouseValuationReports() {
  ...
  const submitWarehouseValuationReport = async (apiUrl) => {
    "use server";
  ...
  return (
    <>
      <PageHead
        pageName={serverLanguage?.csvTranslations?.warehouse_valuation_report ?? "Warehouse Valuation Report"}
        firstName={firstname}
        lastName={lastname}
        initials={initials}
      ...
      />
      <WarehouseValuationReport submitWarehouseValuationReport={submitWarehouseValuationReport} storeCode={storeCode} />
    </>
  );
}
