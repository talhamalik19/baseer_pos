...
import ShippedReport from "@/components/blocks/Reports/ShippedReport";
...
export default async function SalesReports() {
  ...
  const handleShippedReport = async (apiUrl) => {
    "use server";
  ...
  return (
    <>
      <PageHead
        pageName={
          serverLanguage?.csvTranslations?.shipped_report ?? "Shipped Report"
        }
        firstName={firstname}
        lastName={lastname}
        initials={initials}
      ...
      />
      <ShippedReport submitShippedReport={handleShippedReport} storeCode={storeCode} storeIds={storeIds}/>
    </>
  );
}
