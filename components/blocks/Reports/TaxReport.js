...
export default function TaxReport({ submitTaxReport, storeCode, storeIds }) {
  ...
  const buildUrl = (page = 1) => {
    ...
    return `${storeCode}/V1/pos/sales-tax-report?${params.toString()}`;
  };

  const fetchTaxReport = async (page = 1) => {
    const url = buildUrl(page);
    setLoading(true);
    try {
      const response = await submitTaxReport(url);
...
const downloadPdf = async () => {
  ...
  const url = `${storeCode}/V1/pos/sales-tax-report?${params.toString()}`;
  const response = await submitTaxReport(url);
  const result = response?.body?.[0];
 ...
 Back to Reports</Link>
      ...
      <div className={dashboardTable.orders}>
        <div className={dashboardTable.order_head}>
          <h2 className={dashboardTable.title}>Tax Report</h2>
        </div>
{data.items.length > 0 && (
  <button
    type="button"
