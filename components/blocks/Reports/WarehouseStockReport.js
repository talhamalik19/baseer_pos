...
export default function WarehouseStockReport({ submitWarehouseStockReport, storeCode }) {
  ...
  const buildUrl = (page = 1) => {
    ...
    return `${storeCode}/V1/pos/inverntoryWarehouseStockLevel?${params.toString()}`;
  };
  ...
  const fetchReport = async (page = 1) => {
    ...
    setLoading(true);
    try {
      const response = await submitWarehouseStockReport(url);
      ...
      console.error("Error loading warehouse stock report:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadPdf = async () => {
    ...
    const url = `${storeCode}/V1/pos/inverntoryWarehouseStockLevel?${params.toString()}`;
    const response = await submitWarehouseStockReport(url);
    const result = response?.body?.[0] || {};
    const items = result?.items || [];
    ...
    <div className={styles.page_detail}>
      ...
      <div className={dashboardTable.orders}>
        <div className={dashboardTable.order_head}>
          <h2 className={dashboardTable.title}>Warehouse Stock Report</h2>
        </div>
        {data.items.length > 0 && (
