...
export default function ShippedReport({
  ...
  const buildUrl = (page = 1) => {
    ...
    return `${storeCode}/V1/pos/sales-shipped-report?${params.toString()}`;
  };

  const fetchShippedReport = async (page = 1) => {
    const url = buildUrl(page);
    setLoading(true);
    try {
      const response = await submitShippedReport(url);
      ...
      console.error("Error loading shipped report:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    fetchShippedReport(1);
  };

  const handlePageChange = (nextPage) => {
    if (nextPage < 1) return;
    fetchShippedReport(nextPage);
  };
  ...
  const downloadPdf = async () => {
    ...
    const url = `${storeCode}/V1/pos/sales-shipped-report?${params.toString()}`;
    const response = await submitShippedReport(url);
    const result = response?.body?.[0];
    ...
    // Format overall row dynamically
    ...
    <div className={styles.page_detail}>
        ...
        <div className={styles.filter_grid}>
          <div className={styles.filter_group}>
            ...
            >
              ...
              <option value="shipped">First Invoice Created Date</option>
            </select>
          </div>
      ...
      <div className={dashboardTable.orders}>
        <div className={dashboardTable.order_head}>
          <h2 className={dashboardTable.title}>Shipped Report</h2>
        </div>
        {data.items.length > 0 && (
          <button
            type="button"
