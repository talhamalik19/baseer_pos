...
export default function CouponReport({
  ...
  const buildUrl = (page = 1) => {
    ...
    return `${storeCode}/V1/pos/sales-coupon-report?${params.toString()}`;
  };

  const fetchCouponReport = async (page = 1) => {
    ...
    try {
      ...
      setCurrentDataPeriod(formData.period); // ✅ SET PERIOD
      ...
      console.error("Error loading coupon report:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className={styles.page_detail}>
      ...
      <form onSubmit={handleSubmit} className={styles.filter_section}>
        ...
        <div className={styles.filter_grid}>
          <div className={styles.filter_group}>
            ...
            <p className={styles.info}>The Order Updated report is created in real time and does not require a refresh.</p>
          </div>
          ...
          <div className={styles.filter_group}>
            ...
            <p className={styles.info}>Applies to Any of the Specified Order Statuses except canceled and pending orders</p>
          </div>
      ...
      <div className={dashboardTable.orders}>
        <div className={dashboardTable.order_head}>
          <h2 className={dashboardTable.title}>Coupon Report</h2>
        </div>

        {data.items.length > 0 && (
          <button type="button" onClick={downloadPdf} className={styles.export_button} style={{ marginBottom: "10px" }}>
            Download PDF
          </button>
        )}
        ...
        <div className={dashboardTable.table_block}>
          <table>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={columns.length} className={dashboardTable.loading}>Loading...</td>
                </tr>
                ...
                data.overall_totals ? (
                  ...
                  <tr>
                    <td colSpan={columns.length} className={dashboardTable.no_records}>No records found.</td>
                  </tr>
                )
              ) : data.items?.length > 0 ? (
