...
export default function ChangePassword({ handleSubmit, serverLanguage, style }) {
  ...
  const validateForm = () => {
    ...
    if (!newPassword) {
      newErrors.newPassword = serverLanguage?.password_required ?? "New password is required";
    }
    
    if (!confirmPassword) {
      newErrors.confirmPassword = serverLanguage?.confirm_password_required ?? "Please confirm your password";
    } else if (confirmPassword !== newPassword) {
      newErrors.confirmPassword = serverLanguage?.passwords_not_match ?? "Passwords do not match";
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    setMessage(data?.message ?? "Something went wrong. Please try again.");
      ...
      setMessage(serverLanguage?.something_went_wrong ?? "Something went wrong");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className={style.form_block}>
        ...
        >
          ...
          <div className={style.input_box}>
            ...
            {errors.newPassword && <p className={style.error_message}>{errors.newPassword}</p>}
          </div>
          ...
          <div className={style.input_box}>
            ...
            {errors.confirmPassword && <p className={style.error_message}>{errors.confirmPassword}</p>}
          </div>
